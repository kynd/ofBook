...
openFrameworksという試みには導きとなる幾つかの目標があります。コラボラティブであること。シンプルで使えるものであること。一貫性があって直感的であること。クロスプラットフォームであること。パワフルであること。そして拡張性があること。「do it with others（DIWO: 他の人と協力して物事を成し遂げる）」もopenFrameworksを推進する哲学です。
...
openFrameworksの共同作業によって開発されています。度重なる議論に参加してくれる多くの人々の貢献や、アドオンやプロジェクトにおける共同作業によって開発が進んでいます。私たちはopenFrameworksを人々が自分自身のものとし、エコシステムに貢献するよう後押ししています。
...
私たちは分散型バージョン管理システムのgitを使っているので、誰でもブランチを作り実験をしたり、提案をしたりすることができます。[GitHub](https://github.com/openframeworks/openFrameworks "link to the GitHub repository of openFrameworks")のネットワークダイアグラムは、編み物のように絡み合ったブランチ、分かれては繋がるコードでいっぱいのまるで宇宙人が書いた図ように見えます。
コアとなるコードに取り組む世界中に広がるコミュニティがあり、バグを修正し、pullリクエストを送り、自分たちが欲しいツールを自分自身で形作っています。ワールドワイドなプロジェクトなので、朝起きるとメールボックスがアジアやヨーロッパのコーダーたちからのpullリクエストでいっぱいということもよくあります。
...
openFrameworksは使いやすさとシンプルさのバランスを取ろうと試みています。
